...
def summary_from_url(request):
    if request.method == 'POST':
        ...
        #여기에 자막다운로드 기능 추가하기
        title, text = caption_extract(url_link)
        ...
        summary_obj = Summary(user_email=post_email, url_link=url_link, summary=full,title=title, created_at =datetime.datetime.now())
        summary_obj.save()
        print(f'summary fin!, 요약 소요 시간 : {bert_time}초')

        # 응답 처리
        return JsonResponse({'message': 'Data sent and response received successfully', 'title':title, 'summary': full, 'url': url_link, 'bert_time': bert_time})
